...
- [ ] **Unificar el rendimiento de la talla M** — Arma tu Torta dice "10 a 12 porciones" (`lib/armaTuTortaPricing.ts`, `TAMANIOS`) y el catálogo dice "14 a 16 personas" (`DESC_TALLA` en `components/ServicioDetalle.tsx`). Definir una cifra única (S/M/L/XL) y alinear ambos sitios; ver también el pendiente de medir porciones reales por talla.
- [ ] **Revisar el plazo "Entrega en 48 horas después de contactarnos"** en el hero de Arma tu Torta (`components/ArmaTuTorta.tsx`); confirmar que es el plazo real de producción.
- [x] **Decidir si `/arma-tu-torta` lleva Header** — decidido 23-09-2026: las landings, incluida `/arma-tu-torta`, no llevan Header porque tienen salidas propias (ver CHANGELOG, "Navegación").
